...
bool    Bureaucrat::singForm(Form &Form){
  
    if(Form.beSigned(*this) == true){
        std::cout << "\033[36m" << getName() << " ha firmato il documento " << Form.getModulo() << "\033[37m" << std::endl;
        return(true);
    }
    else if (Form.beSigned(*this) == false)
    {
        std::cout << "\033[31m" << getName() << " non ha potuto firmare il documento " << Form.getModulo() << std::endl;
